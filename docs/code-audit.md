...
| AUD-CODE-002 | High | Provenance | Provenance previously existed only at framework level. `School`, `SubjectCombination` and `SchoolOffering` did not store per-record source, checked date or verification status. | `backend/accounts/migrations/0010_school_provenance.py`; `backend/guidance/migrations/0006_catalogue_provenance.py`; `backend/guidance/serializers.py` | The application could not prove which school offering was checked, when it was checked or whether it was official. | Implemented at the schema and public-read boundary. A controlled verification workflow is still required before administrators may publish new authoritative records. | `MITIGATED`; provenance is stored and unverified records are excluded publicly. Operational publishing workflow remains open. |
