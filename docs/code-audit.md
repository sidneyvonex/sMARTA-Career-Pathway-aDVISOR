...
| AUD-CODE-005 | High | Subject catalogue | Grade 10 Core/Essential Mathematics are stored as `Elective`, while Physical Education is stored as `Core`. KICD’s December 2025 Grade 10 addendum defines English, Kiswahili/KSL, Core or Essential Mathematics and CSL as the four core learning areas, with Physical Education scheduled separately. | `backend/students/migrations/0005_correct_grade10_catalogue.py:9`; KICD Grade 10 addendum checked 2026-07-31 | Learners, reports and eligibility logic can present the wrong core/elective classification. The current combination validator also depends on Mathematics being labelled elective, so a direct data edit could break valid combinations. | Separate curriculum status from “selectable in a combination,” correct the catalogue through a new migration and add regression tests based on the verified KICD structure. | `CONTRADICTED` by current Tier 1 KICD material. |
...
- Focused backend guidance/catalogue/model suite: **69 passed** on 2026-07-31.
