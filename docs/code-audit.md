...
| AUD-CODE-002 | High | Provenance | Provenance previously existed only at framework level. `School`, `SubjectCombination` and `SchoolOffering` did not store per-record source, checked date or verification status. | Provenance migrations; official identity/catalogue/offering fetch and import commands; system-admin evidence columns. | The application could not prove which school offering was checked, when it was checked or whether it was official. | Implemented: checksum-pinned, dry-run-first imports; per-record provenance; exact identity matching; quarantine; public verification gates; administrative evidence visibility. | `FIXED`; 1,253 identities, 511 combinations and 10,263 offerings validated and imported locally on 2026-07-31. Production import remains a deployment operation. |
...
| 2026-07-31 | AUD-CODE-002 | Added reproducible Ministry snapshots for 1,253 schools, all 511 combinations and 10,263 exact offering relationships; checksum validation; checkpoint/resume; dry-run/apply importers; quarantine rules; and administrator evidence visibility. | Focused data-pipeline tests, Django checks, migration check and local presentation import passed. |
