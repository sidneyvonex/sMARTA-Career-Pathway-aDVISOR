...
**Status:** Reviewed claims remediated; ongoing source-freshness checks required
...
| AUD-CONTENT-003 | `frontend/src/components/landing/LandingNotice.tsx:81` | Learner and parent | Final placement weighs choices, KJSEA performance, equity and school capacity. | No cohort-specific primary source was stored with the claim. | `FIXED` | Placement criteria can change by cohort; incomplete wording may influence real choices. | Removed the factor list. The page now says the Ministry handles placement and directs users to confirm current criteria, deadlines and results on official services. |
...
- **AUD-CONTENT-003:** removed the unsourced placement-factor list and replaced
  it with a cohort-safe direction to confirm current Ministry information.
